...
import {ProductoRowVm, ProductosApiService} from '../../services/productos-api.service';
import {GridRequest, GridResult, SortDir} from "../../../../../shared/models/grid.model";
import {ToastService} from "../../../../../shared/services/toast.service";

type EstadoFiltro = 1 | 0 | -1; // -1 = Todos
...
export class ProductosListaComponent implements OnInit {
  ...
  // ✅ grid state
  grid: GridResult<ProductoRowVm> = {current: 1, rowCount: 10, total: 0, rows: []};
  ...
  cambiarEstadoFiltro(ev: Event): void {
    ...
    this.estado = Number(value) as any; // 1 | 0 | -1
    this.grid.current = 1;
    this.cargar();
  }
  ...
  toggleEstado(p: ProductoRowVm): void {
    const nuevoEstado = p.estado === 1 ? 0 : 1;

    this.api.cambiarEstado(p.id, nuevoEstado).subscribe({
      next: (res) => {
        if (!res?.success) {
